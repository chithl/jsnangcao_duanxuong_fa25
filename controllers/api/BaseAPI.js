...
export class BaseAPI {
  ...
  async getOne(id) {
    try {
      var response = await axios.get(
        this.baseUrl + this.endpoint + "/" + id + ".json"
      );
      return response;
    } catch (error) {
      return error.response || error;
  ...
  async store(data) {
    try {
      var response = await axios.post(
        this.baseUrl + this.endpoint + ".json",
        data
      );
      return response;
    } catch (error) {
  ...
  /**
   * Gui yêu cầu POST với dữ liệu tùy ý
   * @param {object} data - dữ liệu gửi lên server
   * @returns {Promise<AxiosResponse|any>}
   */
  ...
  async post(path = "", data) {
    const url = path
      ? this.baseUrl + this.endpoint + "/" + path + ".json"
      : this.baseUrl + this.endpoint + ".json";
  ...
  /**
   * Gui yêu cầu PATCH với dữ liệu tùy ý
   * @param {string|number} id - id của bản ghi cần patch
   * @param {object} data - dữ liệu gửi lên server
   * @returns {Promise<AxiosResponse|any>}
   */
